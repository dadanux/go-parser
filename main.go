	...
	process := func(text string) {
		...
		for _, line := range lines {
			if strings.HasPrefix(line, "owner:") {
				   outputLines = append(outputLines, "owner: {{ "+ownerReplacement+" }}")
			...
			if len(m) >= 2 {
			    ...
			    if len(parts) == 2 {
			        outputLines = append(outputLines, "insert_job: {{ prefix }}-"+parts[1])
			    } else {
			        outputLines = append(outputLines, "insert_job: {{ prefix }}-"+fullName)
			    }
			}
  				...
  				if len(m) >= 2 {
  				    ...
  				    parts := strings.SplitN(fullName, "-", 2)
  				    if len(parts) == 2 {
  				        outputLines = append(outputLines, "box_name: {{ prefix }}-"+parts[1])
  				    } else {
  				        outputLines = append(outputLines, "box_name: {{ prefix }}-"+fullName)
  				    }
  				}
...
valEntry.OnFocusLost = func() {
    ...
    ownerEntry := NewMyEntry()
    ownerEntry.SetText(ownerReplacement)
    ...
    ownerBox := container.NewVBox(
        widget.NewLabel("Remplacement pour owner:"),
        ownerEntry,
    )
...
// ✅ Bouton d'export du template



exportBtn := widget.NewButton("Exporter le template", func() {
    saveDialog := dialog.NewFileSave(func(writer fyne.URIWriteCloser, err error) {
        ...
        // Vérifie si le nom contient déjà .j2
        filename := writer.URI().Name()
        if !strings.HasSuffix(filename, ".j2") {
            dialog.ShowError(
                errors.New("Le fichier doit avoir l'extension .j2"),
                w,
            )
            return
        }
        ...
        // Écriture du contenu
        _, err = writer.Write([]byte(output.Text))
        if err != nil {
        ...
        // Confirmation
        dialog.ShowInformation("Export réussi", "Le template a été exporté sous : "+filename, w)
    }, w)

    saveDialog.SetFileName("plan.j2") // nom par défaut
    saveDialog.Show()
})
    ...
    // Intégration des boutons dans la colonne de gauche
    leftTop := container.NewVBox(loadBtn, exportBtn,ownerBox, controlsWithList)
